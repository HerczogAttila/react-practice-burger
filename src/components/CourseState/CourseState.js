...
class CourseState extends Component {
  ...
  render() {
    ...
    const lectures = [...this.state.sections[this.state.section - 13].lectures];
    const actualSectionHour = lectures.splice(this.state.lecture, lectures.length).reduce(
      function (cnt, o) {
        return cnt + o;
      }, 0
    );
        ...
        {this.state.sections.map(section => {
          return section.num < this.state.section ? (
            ...
            <div key={section.num} style={{ color: 'red' }}>
              {section.num}. Section: {(actualSectionHour / 60).toFixed(2)} hour
            </div>
          ) : (
                <div key={section.num}>
